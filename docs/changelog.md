...
- **Tabel Transfer & Stock In: baris expand ke sub-tabel item**: sel kolom **Variant** kini tampil ringkas ("N item" + ikon chevron); klik untuk **expand barisnya**, memunculkan **tabel bersarang** (`UTable` sendiri, dengan header Variant/Branch/Condition/Quantity) di bawah baris — bukan lagi daftar teks bertumpuk. `DataTable.vue` (komponen tabel bersama) diperluas mendukung `v-model:expanded` + slot `#expanded` (fitur native TanStack Table row-expansion).
