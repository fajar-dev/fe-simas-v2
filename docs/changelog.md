...
- **Daftar Persediaan: kolom Variants dihapus, Balance jadi Stock, baris expand tampilkan stok Baru/Bekas per varian**: tabel utama tidak lagi punya kolom **Variants** (jumlah varian sudah tersirat dari baris yang bisa di-expand); kolom **Balance** berganti nama jadi **Stock**. Tabel bersarang pada baris expand tidak lagi punya kolom **Code** terpisah (kode kini ditampilkan di bawah nama varian, sama seperti kolom Name pada tabel utama), dan menambah dua kolom baru **New**/**Used** — total stok baru & bekas per varian, dijumlahkan dari saldo stok lintas cabang.
