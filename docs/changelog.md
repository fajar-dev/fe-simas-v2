...
- **Kolom item pada tabel Transfer & Stock In jadi expand table**: bila dokumen punya >1 item, sel kolom **Variant** kini tampil ringkas ("N item", ikon chevron) dan bisa **di-klik untuk expand/collapse** menampilkan daftar item lengkap (varian, cabang, kondisi, qty) terindentasi — pola interaksi sama seperti tabel **Varian & Stok** (`StockOverview.vue`). Komponen baru `inventory/ItemsExpandCell.vue` dipakai bersama oleh kedua tabel.
