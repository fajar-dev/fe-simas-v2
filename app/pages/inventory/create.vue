          ...
          <div class="space-y-4">
            ...
            <UFormField :label="$t('common.code')" name="code">
              <div class="flex items-center gap-2">
                ...
                <UButton icon="i-lucide-scan" color="neutral" variant="soft" square :title="$t('pages.inventory.create.scan')" @click="() => { openScanner({ kind: 'item' }) }" />
              </div>
            </UFormField>
            ...
            <div v-else class="space-y-3">
              <div v-for="(v, vi) in form.variants" :key="vi" class="p-3 rounded-lg border border-neutral-200 space-y-2.5">
                <div class="flex items-center gap-2">
                  ...
                  <UButton icon="i-lucide-trash" color="error" variant="soft" size="md" square class="mt-6" @click="() => { form.variants.splice(vi, 1) }" />
                </div>
                <UFormField :label="$t('common.code')" :name="`variants.${vi}.code`">
                  <div class="flex items-center gap-2">
                    <UInput v-model="v.code" :placeholder="$t('common.autoGenerated')" class="w-full" />
                    <UButton icon="i-lucide-scan" color="neutral" variant="soft" square :title="$t('pages.inventory.create.scan')" @click="() => { openScanner({ kind: 'variant', index: vi }) }" />
                  </div>
                </UFormField>
              </div>
            </div>
...
interface VariantRow { name: string, code: string }
...
const addVariant = () => { form.variants.push({ name: '', code: '' }) }
...
const onScanned = (code: string) => {
  ...
  if (target.kind === 'item') {
    ...
    else form.variants.push({ name: trimmed, code: trimmed })
  }
}
...
const onSubmit = async () => {
  ...
  try {
    const payloadVariants: InventoryVariantInput[] = form.variants
      ...
      .map(v => ({ name: v.name.trim(), code: v.code.trim() || null }))
