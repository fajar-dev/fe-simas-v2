...
const { t } = useI18n()
const props = defineProps<{ inventoryId: number, refreshKey?: number }>()
...
const fetchBalances = async () => {
  ...
  try {
    const res = await inventoryStockService.getBalances(1, 500, { inventoryId: props.inventoryId })
...
watch(() => props.refreshKey, () => { fetchBalances() })
